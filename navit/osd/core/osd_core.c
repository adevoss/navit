    ...
    while (oti) {
        ...
        if (oti->static_text) {
            ...
            if (item) {
                ...
                dbg(lvl_debug, "type %s", attr_to_name(oti->attr_type));
                if (item_attr_get(item, oti->attr_type, &attr))
            ...
            if (vehicle_attr.u.vehicle) {
                if (vehicle_get_attr(vehicle_attr.u.vehicle, oti->attr_type, &attr, NULL)) {
                    value = osd_text_format_attr(&attr, oti->format, imperial);
                }
            }
            ...
            if (tracking) {
                item = tracking_get_current_item(tracking);
                if (item && (oti->attr_type == attr_speed)) {
                    double routespeed = -1;
                    ...
                    if (tracking_get_attr(tracking, oti->attr_type, &attr, NULL))
                        value = osd_text_format_attr(&attr, oti->format, imperial);
                }
            }

        } else if (oti->section == attr_navit) {
            if (oti->attr_type == attr_message) {
                struct message *msg;
                int len, offset;
                char *tmp;
        ...
        *end++ = '\0';
        ...
        if ((oti->section == attr_navigation || oti->section == attr_tracking) && subkey) {
            ...
            if (!strcmp(key, "route_speed")) {
                oti->attr_type = attr_speed;
            } else {
                oti->attr_type = attr_from_name(key);
            }
            ...
            if (!strcmp(subkey, "messages")) {
                oti->attr_type = attr_message;
            } else {
                oti->attr_type = attr_from_name(subkey);
            }
            oti->format = g_strdup(index);
        }

        switch (oti->attr_type) {
        default:
