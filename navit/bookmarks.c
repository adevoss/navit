...
static void bookmarks_load_hash(struct bookmarks *this_) {
    ...
    while ((item = map_rect_get_item(this_->mr))) {
        if (item->type != type_bookmark && item->type != type_bookmark_folder)
            continue;
        ...
        if (!g_hash_table_lookup(dedup, fullname)) {
            ...
            if (item->type == type_bookmark) {
                ...
                if (fprintf(f, "%s%s%s0x%x %s0x%x type=%s label=\"%s\" path=\"%s\"\n", prostr, *prostr ? ":" : "",
                            ...
                            item->c.y >= 0 ? "" : "-", item->c.y >= 0 ? item->c.y : -item->c.y, "bookmark", item->label,
                            fullname)
                    < 1) {
                    g_free(fullname);
                    break;
            ...
            if (item->type == type_bookmark_folder) {
                prostr = projection_to_name(projection_mg);
                if (fprintf(f, "%s%s%s0x%x %s0x%x type=%s label=\"%s\" path=\"%s\"\n", prostr, *prostr ? ":" : "", "",
                            0, "", 0, "bookmark_folder", item->label, fullname)
                    < 1) {
                    g_free(fullname);
                    break;
                }
...
/*
 * Doesn't read any items besides former_destination. So active waypoints items of type former_itinerary and
 * former_itinerary_part are skipped.
 */
...
/*
 * Find destination in given GList. If remove_found is non-zero, any matching items are removed and new beginning of the
 * list is returned. If remove_found is zero, last matching item is returned. In the latter case, description is ignored
 * and can be NULL.
 */
        ...
        for (currdest = former_destinations; currdest; currdest = g_list_next(currdest)) {
            ...
            do {
                ...
                fprintf(f, "%s%s%s0x%x %s0x%x\n", prostr, *prostr ? ":" : "", c->x >= 0 ? "" : "-",
                        c->x >= 0 ? c->x : -c->x, c->y >= 0 ? "" : "-", c->y >= 0 ? c->y : -c->y);
 ...
 * @param c coordinates of item point(s). Can be set to NULL when navigation is stopped to remove type_former_itinerary
 * and type_former_itinerary_part items from the file.
 ...
 * @param description character string used to identify this destination. If NULL, most recent waypoint at these
